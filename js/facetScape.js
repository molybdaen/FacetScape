function facetScape(domElem, iwidth, iheight, ifacets, queryResultItems, term, session) {
    ...
    var onMouseOverFacetNameFunction = function(d,i) {
    ...
    var onMouseOverFunction = function(d,i) {
        ...
        selRect.style("stroke", "AADDE3");
        var gain = 0;
        var facetName = this.parentNode.__data__.name;
        if(!d.isSelected) {
            selRect.style("fill", "#E1EDF2");
        ...
        for(var i = 0; i < facetData.length; i++) {
            if(facetData[i] != facet) {
                ...
                if(distance < (facetData[i].weight + facet.weight)) {
                    ...
                    collFacet.style("stroke", "#FF7600").style("stroke-width", "3px").transition().duration(1000).style("stroke", "#000000").style("stroke-width", "1px");
                }
            }
        }
        if(isNotOverlapping) {
    ...
    function init() {
        ...
        createDropShadowFilter(0);
        createDropShadowFilter(1);
        createDropShadowFilter(2);
        createDropShadowFilter(3);
        createDropShadowFilter(4);
...
//        var dropdown = resultHeaderSearch.append("select").attr("id", "RS_Query").attr("name", "query");//.attr("value",searchTerm);
//        dropdown.append("option").attr("value", searchTerm).text(searchTerm).attr("selected");
//        dropdown.append("option").attr("value", "Declaration of Independence").text("Declaration of Independence");
//        dropdown.append("option").attr("value", "Paper Machine").text("Paper Machine");
//        dropdown.append("option").attr("value", "Revolution").text("Revolution");
//        dropdown.append("option").attr("value", "Gold Coin").text("Gold Coin");
//        dropdown.append("option").attr("value", "House Bavaria").text("House Bavaria");
//        dropdown.append("option").attr("value", "Steam Locomotive").text("Steam Locomotive");
//        dropdown.append("option").attr("value", "Bridge").text("Bridge");
//        dropdown.append("option").attr("value", "Donau").text("Donau");
        ...
        resultList.append("div").attr("id", "RS_ResultList").style("height", height-svgHeight-headerHeight+"px");
        refreshResultList();
    }
    ...
    function drawVoronoi() {
        ...
        var groupSel = facetNameSelection.enter().append("g").attr("id", "facet_Header_group")
            ...
            .on("mouseover", onMouseOverFacetNameFunction)
            .on("mousewheel.zoom", onMouseWheel);
        ...
        facetNameSelection.selectAll("rect")
    ...
    function drawTagCloud() {
        ...
        tagSelection.select("rect#tag_bg")
            ...
            .style("fill", function(d,i) {return (d.isSelected ? "#B7DBEB" : "none"); });
    ...
    facetScapeObject.visualize = function(data) {
       drawTagCloud();
    }
    ...
    function search(term) {
        ...
        var onReceiveData = function(processedData, items) {
            ...
            facetScape(root, svgWidth, height, processedData, items, queryTerm, session);
        }
